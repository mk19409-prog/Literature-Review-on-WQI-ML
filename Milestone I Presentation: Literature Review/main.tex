...
Machine learning improves water quality assessment by modeling complex and nonlinear relationships that can adapt in different geographic, seasonal, and hydrological conditions \cite{jayaraman2024critical}. Different ML models (such as Decision Trees, Random Forests, Support Vector Machines, Regression, and Gradient Boosting) and DL models (Artificial Neural Networks, Convolutional Neural Networks, Recurrent Neural Networks, Long Short-Term Memory networks, and transformers) are widely used for the prediction and forecasting of the water quality index \cite{han2025predicting,ehteram2024advanced}. In addition, multi-task learning frameworks allow the simultaneous prediction of multiple WQIs for different water uses by sharing common feature representations while generating task-specific outputs, improving scalability and generalization across diverse contexts \cite{uddin2022robust}.
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=.99\columnwidth]{figures/pi.pdf}
    \caption{Overall Pipeline of the Study Design}
    \label{fig:main}
\end{figure}



The whole process has multiple steps, such as finding keywords, accessing databases, initial screening, and using inclusion and exclusion criteria \cite{syeed2023surface}, as depicted in Figure \ref{fig:main}.

\begin{enumerate}[{(1)}]
\item \textbf{Keyword Used}
To find the relevant research papers, the first procedure is to define the appropriate keyword searches.  The following combinations of keywords have been used:
...
\item {\textbf{Databases Accessed:}}
We used several well-known scholarly databases to find reputable, research-oriented publications. These types of databases publish peer-reviewed and trustworthy scientific research. The databases used for the current study include the following:
\begin{itemize}
    \item ScienceDirect
    \item Scopus
    \item IEEE Xplore
...
\item \textbf{Initial Paper Identification}
...
\item \textbf{Screening of Relevant Research Papers}
...
\begin{itemize}
    ...
    \item Initial selection Research papers: 48, selected based on our research (WQI): 40, finally selected: 16 journals (WQI related).
...
\item \textbf{Final Paper Selection}
...
\caption{Exclusive Summary of Selected Papers}
...
Recent research on Water Quality Index (WQI) prediction shows a strong transition from conventional weighted arithmetic models toward machine learning (ML), deep learning (DL), and hybrid ensemble approaches \cite{mo2024assessment}. Across the reviewed papers, the central problem is the limitation of traditional WQI frameworks, including rigid parameter weighting, inconsistent classification schemes, region-specific formulations, and poor generalization across environmental contexts \cite{zhu2022review, ehteram2024advanced}. An exclusive summary of the literature review with most crucial seven papers are presented in Table \ref{tab:lit-review}. A detailed literature review can be found from the access link to the \href{https://docs.google.com/spreadsheets/d/1uNeQQFx8vY5-hyzwO9BjR6l4s6zKMueg/edit?usp=sharing\&ouid=106348839836730303077\&rtpof=true\&sd=true}{Google Sheet}.
...
Input features typically included physicochemical water quality parameters such as pH, DO, BOD, COD, EC, TDS, turbidity, sulfate, hardness, salinity, and sometimes climate indicators. However, except catchment dataset, many datasets were small and geographically limited \cite{uddin2023performance, singh2024predictive}.
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/sync.jpg}
\caption{Intuition and Synthesis from the Literature Review (Framework Diagram).}
\label{fig:sync}
\end{figure*}

However, this literature review identified the following set of research questions:

\begin{itemize}
    \item How do machine learning models improve the prediction and classification of Water Quality Index (WQI) compared to traditional statistical methods?
    
    \item Can a generalized Water Quality Index model be developed that supports multiple water-use purposes (e.g., drinking, irrigation, ecosystem health)?
    
    \item Which machine learning approaches improve model generalization and efficiency by leveraging shared feature representations across multiple WQI models?
    
    \item How can explainable artificial intelligence (XAI) methods be integrated into machine learning-based water quality assessment?
    
\end{itemize}


\section{Selected Dataset for the Project}

For this project we will utilize water quality dataset available on Catchments, i.e. \cite{ds}, that provides comprehensive information on Ireland’s rivers, lakes, groundwater, estuaries, and coastal waters, assessed under the EU Water Framework Directive (WFD) \cite{epa}. This standardized data collection ensures reliability and supports transferability to other regions with minimal recalibration. 


\begin{figure}[h!]
    \centering
    \includegraphics[width=.99\columnwidth]{figures/data_summary_new.png}
    \caption{Data Collection Summary}
    \label{fig:data}
\end{figure}

We collected initial data from the specific regions in Ireland, especially the Cork Harbour, Moy \& Killala, and the other random regions, was mainly informed by the presence of complete parameters, long-term, and good-quality water records. The areas are highly varied in terms of hydro-environmental conditions, which include the coastal and riverine ecosystems, hence will provide a perfect testbed in assessing the strength of the universal non-region specific project framework. 

The gathered data has a total of 470,504 records that have been gathered within 17 years, 2008-2024. This long duration gives an extensive coverage of the number of water quality classification combinations that are possible, including seasonal changes as well as long-term changes. Also, in every entry, there is data on 10 different parameters of water quality, as shown in Figure \ref{fig:data}. The parameters were chosen according to their relevance, correlation with one another, availability, and compatibility with the WQI models referenced at \cite{uddin2021review}. As such, the dataset is comprehensive, contextually significant, and well-suited for the objectives of this research.

\section{Tentative Method Planned for the Project}

A simple or hybrid machine learning model will be used to predict application and user based WQI and classify water quality for different purposes, as depicted in Figure \ref{fig:proj}. 

\begin{figure}[h!]
    \centering
    \includegraphics[width=.99\columnwidth]{figures/diagram_proj.jpg}
    \caption{Tentative Multi-task ML Project Framework}
    \label{fig:proj}
\end{figure}
